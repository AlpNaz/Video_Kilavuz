...
import com.project.videoklavuz.SampleApplication.SampleAppRendererControl;
import com.project.videoklavuz.SampleApplication.SampleApplicationSession;
import com.project.videoklavuz.SampleApplication.utils.SampleMath;
...
import com.project.videoklavuz.SampleApplication.SampleAppRenderer;
import com.project.videoklavuz.SampleApplication.utils.SampleUtils;
import com.project.videoklavuz.SampleApplication.utils.Texture;


// The renderer class for the VideoOynatici sample.
public class Renderer implements GLSurfaceView.Renderer, SampleAppRendererControl
{
    ...
    int videoPlaybackTextureID[] = new int[VideoOynatici.NUM_TARGETS];
    ...
    Vec3F targetPositiveDimensions[] = new Vec3F[VideoOynatici.NUM_TARGETS];
    ...
    private VideoHelper.MEDIA_TYPE mCanRequestType[] = null;
    private int mSeekPosition[] = null;
    private boolean mShouldPlayImmediately[] = null;
    private long mLostTrackingSince[] = null;
    private boolean mLoadRequested[] = null;

    VideoOynatici mActivity;

    // Needed to calculate whether a screen tap is inside the target
    Matrix44F modelViewMatrix[] = new Matrix44F[VideoOynatici.NUM_TARGETS];

    private Vector<Texture> mTextures;

    boolean isTracking[] = new boolean[VideoOynatici.NUM_TARGETS];
    VideoHelper.MEDIA_STATE currentStatus[] = new VideoHelper.MEDIA_STATE[VideoOynatici.NUM_TARGETS];

    // These hold the aspect ratio of both the video and the
    // keyframe
    float videoQuadAspectRatio[] = new float[VideoOynatici.NUM_TARGETS];
    float keyframeQuadAspectRatio[] = new float[VideoOynatici.NUM_TARGETS];


    public Renderer(VideoOynatici activity,
                    SampleApplicationSession session)
    {
        ...
        mVideoHelper = new VideoHelper[VideoOynatici.NUM_TARGETS];
        mMovieName = new String[VideoOynatici.NUM_TARGETS];
        mCanRequestType = new VideoHelper.MEDIA_TYPE[VideoOynatici.NUM_TARGETS];
        mSeekPosition = new int[VideoOynatici.NUM_TARGETS];
        mShouldPlayImmediately = new boolean[VideoOynatici.NUM_TARGETS];
        mLostTrackingSince = new long[VideoOynatici.NUM_TARGETS];
        mLoadRequested = new boolean[VideoOynatici.NUM_TARGETS];
        mTexCoordTransformationMatrix = new float[VideoOynatici.NUM_TARGETS][16];

        // Initialize the arrays to default values
        for (int i = 0; i < VideoOynatici.NUM_TARGETS; i++)
        {
            mVideoHelper[i] = null;
            mMovieName[i] = "";
            mCanRequestType[i] = VideoHelper.MEDIA_TYPE.ON_TEXTURE_FULLSCREEN;
            mSeekPosition[i] = 0;
    ...
    public void setVideoPlayerHelper(int target,
                                     VideoHelper newVideoHelper)
    {
        mVideoHelper[target] = newVideoHelper;
    }
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config)
    {
        ...
        for (int i = 0; i < VideoOynatici.NUM_TARGETS; i++)
        {
            ...
            {
                // The VideoHelper needs to setup a surface texture given
                // the texture id
                ...
                if (!mVideoHelper[i]
                    ...
                    mCanRequestType[i] = VideoHelper.MEDIA_TYPE.FULLSCREEN;
                else
                    mCanRequestType[i] = VideoHelper.MEDIA_TYPE.ON_TEXTURE_FULLSCREEN;
                ...
                if (mLoadRequested[i])
                {
                    mVideoHelper[i].load(mMovieName[i],
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height)
    {
        ...
        for (int i = 0; i < VideoOynatici.NUM_TARGETS; i++)
        {
            if (mLoadRequested[i] && mVideoHelper[i] != null)
            {
                mVideoHelper[i].load(mMovieName[i], mCanRequestType[i],
        ...
        for (int i = 0; i < VideoOynatici.NUM_TARGETS; i++)
        {
            if (mVideoHelper[i] != null)
            {
                if (mVideoHelper[i].isPlayableOnTexture())
                {
                    ...
                    if (mVideoHelper[i].getStatus() == VideoHelper.MEDIA_STATE.PLAYING)
                    {
                        mVideoHelper[i].updateVideoData();
                    }
                    ...
                    setVideoDimensions(i,
                            mVideoHelper[i].getVideoWidth(),
                            mVideoHelper[i].getVideoHeight(),
                            mTexCoordTransformationMatrix[i]);
                }

                setStatus(i, mVideoHelper[i].getStatus().getNumericType());
            }
        }
    ...
    void initRendering()
    {
        Log.d(LOGTAG, "VideoOynatici Renderer initRendering");
        ...
        /*keyframeQuadAspectRatio[VideoOynatici.STONES] = (float) mTextures
            .get(0).mHeight / (float) mTextures.get(0).mWidth;
        keyframeQuadAspectRatio[VideoOynatici.CHIPS] = (float) mTextures.get(1).mHeight
    ...
    public void renderFrame(State state, float[] projectionMatrix)
    {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++)
        {
            ...
            // calculation
            if (imageTarget.getName().compareTo("Kapi_Acik") == 0)
                currentTarget = VideoOynatici.Kapi_Acik;
            else if (imageTarget.getName().compareTo("Elektrik_Destekli_Direksiyonun_Devreye_Alinmasi") == 0)
                currentTarget = VideoOynatici.Elektrik_Destekli_Direksiyonun_Devreye_Alinmasi;
            else if (imageTarget.getName().compareTo("El_Freni_Cekili") == 0)
                currentTarget = VideoOynatici.El_Freni_Cekili;
            else
                currentTarget = VideoOynatici.City_Dugmesi;
            ...
            if ((currentStatus[currentTarget] == VideoHelper.MEDIA_STATE.READY)
                    || (currentStatus[currentTarget] == VideoHelper.MEDIA_STATE.REACHED_END)
                    || (currentStatus[currentTarget] == VideoHelper.MEDIA_STATE.NOT_READY)
                    || (currentStatus[currentTarget] == VideoHelper.MEDIA_STATE.ERROR))
            {
            ...
            if ((currentStatus[currentTarget] == VideoHelper.MEDIA_STATE.READY)
                    || (currentStatus[currentTarget] == VideoHelper.MEDIA_STATE.REACHED_END)
                    || (currentStatus[currentTarget] == VideoHelper.MEDIA_STATE.PAUSED)
                    || (currentStatus[currentTarget] == VideoHelper.MEDIA_STATE.NOT_READY)
                    || (currentStatus[currentTarget] == VideoHelper.MEDIA_STATE.ERROR))
            {
            ...
            SampleUtils.checkGLError("VideoOynatici renderFrame");
        }
                            ...
                            float[] textureCoordMatrix)
    {
        ...
        if (target == VideoOynatici.Kapi_Acik)
        {
        ...
        } else /*if (target == VideoOynatici.CHIPS)*/
        {
            tempUVMultRes = uvMultMat4f(
    ...
    void setStatus(int target, int value)
    {
        ...
        {
            case 0:
                currentStatus[target] = VideoHelper.MEDIA_STATE.REACHED_END;
                break;
            case 1:
                currentStatus[target] = VideoHelper.MEDIA_STATE.PAUSED;
                break;
            case 2:
                currentStatus[target] = VideoHelper.MEDIA_STATE.STOPPED;
                break;
            case 3:
                currentStatus[target] = VideoHelper.MEDIA_STATE.PLAYING;
                break;
            case 4:
                currentStatus[target] = VideoHelper.MEDIA_STATE.READY;
                break;
            case 5:
                currentStatus[target] = VideoHelper.MEDIA_STATE.NOT_READY;
                break;
            case 6:
                currentStatus[target] = VideoHelper.MEDIA_STATE.ERROR;
                break;
            default:
                currentStatus[target] = VideoHelper.MEDIA_STATE.NOT_READY;
                break;
